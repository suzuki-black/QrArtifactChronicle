...
> ⚠️ **実装状況**: 本章は**設計目標**。現行 `qrac-render/compose.rs` は **手続き生成のプレースホルダ**で、
> キャンバスは **512²**、汚れ/破損は手続き描画（**透過PNGレイヤー/ブレンドモード・LRUキャッシュ・1024²は未実装**）。
> 実装の現況は docs/08 §8.6 を参照。HSV(層2)とベース画像差し替え可能性は実装済み。
